...
指数行情页在“资金流动”右侧提供独立的“市场情绪”切换按钮。该视图展示 60 秒真实采样的 A 股市场情绪曲线，以双纵轴同时呈现涨停、跌停、炸板、红盘和绿盘数量；对应的 FastAPI 接口为 `/api/market_breadth`，深链接使用 `/indices?panel=market-breadth`。
