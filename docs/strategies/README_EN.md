...
Administrator backtest v34 includes the terminal liquidation session after the signal window in the equity curve and risk statistics, and keeps long-session timing plus ETA current. Older backtest results are marked stale and must be rerun; selection, fill, sizing, and risk rules are unchanged.
