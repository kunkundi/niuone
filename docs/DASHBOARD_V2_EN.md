...
The indices page adds a dedicated **Market Sentiment** switch immediately after **Fund Flow**. This view presents a 60-second real-sample A-share curve whose dual axes display limit-ups, limit-downs, broken limit-ups, red stocks, and green stocks through the FastAPI-native `/api/market_breadth` endpoint. Its deep link is `/indices?panel=market-breadth`.
