...
| `DASHBOARD_NIUONE_FORWARD_COHORT_START` | `2026-08-13` | 严格前向队列起始日；修改规则时归档旧协议锁并从新交易日重新累计 |
...
管理员回测 v34 将信号期后的最终平仓日计入权益曲线及风险指标，并改进长耗时回放的当前交易日计时和剩余时间估算。独立部署升级后旧回测结果会失效并要求重跑；策略规则、成交精度和资金计算不变。
