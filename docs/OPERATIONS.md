...
指数行情页的 A 股市场情绪曲线每 60 秒读取一次腾讯证券沪深 A 股全市场快照，并用行情返回的现价、最高价、涨停价和跌停价计算涨停板、跌停板与炸板数量；红盘、绿盘按行情涨跌幅正负统计。统计口径包含 ST，不含 B 股、北交所及无有效现价证券。后台只在 A 股交易日 09:30～11:30、13:00～15:00 采样，真实点保存在 `market_breadth_history.json`；分片不完整或请求失败时保留上一份有效历史，不写入伪零值。
